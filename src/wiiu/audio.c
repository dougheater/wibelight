...
// Audio buffer size (SDL want.samples) — exposed for UI tuning
// Presets: 2048 (default, lower latency) or 4096 (more stable, higher latency)
int audio_buffer_samples = 4096;
...
static void sdl_renderer_decode_and_play_sample(char* data, int length) {
  int decodeLen = opus_multistream_decode(decoder, (const unsigned char *)data, length, pcmBuffer, samplesPerFrame, 0);
  if (decodeLen > 0) {
