...
// Benchmark state (from main.c)
extern int benchmark_running;
extern int benchmark_decode_errors;
extern uint64_t benchmark_total_bytes;
extern int benchmark_total_frames;
...
static int wiiu_decoder_submit_decode_unit(PDECODE_UNIT decodeUnit) {
  ...
  // Track bytes for benchmark average bitrate calculation
  if (benchmark_running) {
    benchmark_total_bytes += length;
  }

  int res = H264DECSetBitstream(decoder, (uint8_t *)decodebuffer, length, 0);
  if (res != 0) {
    printf("h264_wiiu: Error setting bitstream 0x%07X\n", res);
    if (benchmark_running) benchmark_decode_errors++;
    return DR_NEED_IDR;
  }
