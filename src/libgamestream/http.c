...
int http_request(HTTP *http, char *url, HTTP_DATA *data) {
    ...
    if (res != CURLE_OK) {
        ...
        // If FAILONERROR triggered, fetch the actual HTTP status code
        if (res == CURLE_HTTP_RETURNED_ERROR) {
            long http_code = 0;
            curl_easy_getinfo(curl, CURLINFO_HTTP_CODE, &http_code);
            ret = gs_set_error(GS_IO_ERROR, "HTTP error %ld: %s", http_code, errmsg);
        } else if (res == CURLE_OPERATION_TIMEDOUT) {
            ret = gs_set_error(GS_IO_ERROR, "Connection timed out");
        } else if (res == CURLE_COULDNT_CONNECT) {
            ret = gs_set_error(GS_IO_ERROR, "Connection refused — is the host running?");
        } else if (res == CURLE_COULDNT_RESOLVE_HOST) {
            ret = gs_set_error(GS_IO_ERROR, "Could not resolve hostname");
        } else if (res == CURLE_SSL_CONNECT_ERROR) {
            ret = gs_set_error(GS_IO_ERROR, "SSL connection failed");
        } else {
