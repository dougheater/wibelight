...
static int load_server_status(GS_CLIENT hnd, PSERVER_DATA server) {
    ...
    do {
        ...
        if (!construct_url(hnd, url, sizeof(url), i == 0, server->serverInfo.address, "serverinfo", NULL)) {
            gs_set_error(GS_IO_ERROR, "construct_url failed (uuid generation error)");
            ret = GS_IO_ERROR;
            goto cleanup;
        }
...
int gs_unpair(GS_CLIENT hnd, PSERVER_DATA server) {
    ...
    // Sunshine does not implement /unpair. For Sunshine hosts, clear paired
    // locally and let the user unpair via the host's web UI if needed.
    if (!server->isGfe) {
        server->paired = false;
        goto cleanup;
    }
